...
| **Dunder Methods & Domain Helpers** | Custom `str()` (`__str__`) and `combine_budget()` helper on `ProjectBudget` | [models/project_budget.py#L110-L125](file:///d:/Center/Github_Profile/Research-Supply-Chain/addons/research_supply_chain/models/project_budget.py#L110-L125) |
| **Managed Properties** | `@property` getter (`utilization_percentage`), setter, and deleter | [models/project_budget.py#L135-L150](file:///d:/Center/Github_Profile/Research-Supply-Chain/addons/research_supply_chain/models/project_budget.py#L135-L150) |
...
| **Generators & Itertools** | Generator streaming (`yield`) and `itertools.groupby` for API experiment classification | [controllers/main.py#L55-L70](file:///d:/Center/Github_Profile/Research-Supply-Chain/controllers/main.py#L55-L70) |
...
### 2. Dunder Methods & Domain Helpers (`models/project_budget.py`)

```python
# Friendly string formatting for singletons while keeping Odoo ORM recordset identity safe
def __str__(self) -> str:
    if len(self) == 1:
        return f"ProjectBudget(Project={self.project_id.project_name}, Total=${self.total_amount:,.2f})"
    return super().__str__()

# Helper method to calculate combined totals between two project budgets
